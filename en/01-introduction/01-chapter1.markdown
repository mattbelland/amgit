...
1.	I am frustrated most existing books about Git because they talk too much about how Git is implemented, and not enough about the mental models users need to use it.
...
4.	And most existing books are not organized around the use cases beginners are most likely to see.
...
Suppose you are writing a paper.  Maybe you keep the files related to the paper in a folder on a hard drive in your computer.  This folder probably contains the text of the paper, which might be split up into a few files, along with some data files, programs that process the data, figures and tables generated by the programs, and maybe files that contain your notes and related work.  If there are a large number of files, you might have them organized in sub-folders.
...
*	You might have copies of your files on a computer at work, a laptop, and a computer at home.  When you make changes, you have to propagate the changes from one copy to the others, and it is easy to lose track of which changes are in which copies.
...
*	You probably want your collaborators to be able to read and edit your files.  And you might want some other people to read, but not edit.  And you might want to prevent others from reading, or you might want to make it easy for others to discover your work.
...
One way to address all of these problems is by using a version control system (VCS).  Version control systems provide tools that allow you to store your files into a named repository.  A repository is a collection a collection of files, similar to a folder on a hard drive, but with several additional capabilities.   In particular, 

*	A repository keeps track of the history of its files, so it is easy to see who changed what, and when, and why.  *	And it is easy to see older versions of the files, and compare any two versions, and if you mess something up, you can go back to an older version.
